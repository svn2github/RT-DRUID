	...
	protected void writeImplementation(EPackage ePackage, EClass parent_class, IOilImplPointer oip, ArrayList<String> path) {
		...
		switch (cd.getType()) {
			...
			default:
				...
				OilModelLog.showDebug("Check " + getClass().getName() +" for unused " + IOilImplPointer.class.getName() + " types ("+cd.getType()+")");
				//break;
...
//				
	...
	private EClass getRtdEClass(String type) {
		...
		if (IOilXMLLabels.OBJ_TASK.equalsIgnoreCase(type)) {
			...
			answer = DataPackage.Literals.SIGNAL;
		} else if (IOilXMLLabels.OBJ_APPMODE.equalsIgnoreCase(type)) {
			answer = DataPackage.Literals.MODE;
		}
		return answer;
	}
		...
		for (PendingRefs pendingRef: pendingReferences) {
			...
			if (ec != null) {
				...
				if (pendingRef.type.endsWith("_TYPE")) {
					...
					OilModelLog.showDebug("Trying "+new_type + " instead of " + pendingRef.type+ "\n");
				...
				if (ec != null) {
					...
					OilModelLog.showDebug("Cannot find required reference : " + pendingRef.type+ "\n");
				}
			}
	...
	private static EClassifier computeEType(String type) {
		if (EMF_TYPES == null) {
