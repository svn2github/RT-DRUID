 ...
 */
public class SectionWriterHalI386 extends SectionWriter 
	 ...
	 * 
	 */
	public SectionWriterHalI386(ErikaEnterpriseWriter parent) {
		super(IWritersKeywords.CPU_X86,
				new String[] {}, //
				new String[] { //
		...
		isrWriter = new SectionWriterIsr(parent, IWritersKeywords.CPU_X86);
		...
		for (int currentRtosId = 0; currentRtosId < oilObjects.length; currentRtosId++) {
			...
			
			{
				 ...
				 **********************************************************************/
				{
			        ...
			        tmp_eeopts.add("__X86__");
			        
			        {
			        	List<String> all = parent.getCpuDataEnum(ool, "USE_UART");
						String tmp1 = all.size() == 0? null : all.get(0);
						if ("TRUE".equalsIgnoreCase(tmp1)) {
							tmp_eeopts.add("__USE_UART__");
				        }
					}
		
					{
						...
						String tmp1 = all.size() == 0? null : all.get(0);
						if (tmp1 == null) {
							tmp1 = X86Constants.SGRK__GCC_COMPILER__;
						}
							
						sgrCpu.setProperty(X86Constants.SGRK__COMPILER_TYPE__, tmp1);
						if (X86Constants.SGRK__GCC_COMPILER__.equalsIgnoreCase(tmp1)) {
							tmp_eeopts.add("__GCC__");
					    } else 	if (X86Constants.SGRK__CLANG_COMPILER__.equalsIgnoreCase(tmp1)) {
					    	tmp_eeopts.add("__CLANG__");
					    }
	 ...
	 */
	protected IOilWriterBuffer[] writeEE_x86_CPU()
			throws OilCodeWriterException {
...
//						+ "\n\n");
	...
	protected StringBuffer handleStacks(int currentRtosId, final IOilObjectList ool) throws OilCodeWriterException  {
		...
		EEStackData sys_stack = new EEStackData(0,
				...
				new long[] {0},
				new String[] {" (int)&EE_x86_sys_stack "}, true);
...
//			StringBuffer stackPatternFill = new StringBuffer();
			...
			sbInithal_c.append(commentWriterC
					.writerBanner("Stack definition for X86"));
...
//				    value  = (value + (value%STACK_UNIT)) / STACK_UNIT; // arrottondo a 2
				...
				// open system tos
				sbStack.append(indent
						+ "struct EE_TOS EE_std_system_tos["+ErikaEnterpriseWriter.addVectorSizeDefine(ool, "EE_std_system_tos", tos_size)
						+ "] = {\n");
				...
				for (int j = 0; j < tos_size; j++) {
			        ...
			        String value = j == 0 ? "{0}" : "{(EE_ADDR)(&"+STACK_BASE_NAME+j+"[STACK_"+j+"_SIZE - X86_INIT_TOS_OFFSET])}"; // DELTA
...
//					    value  = (value + (value%STACK_UNIT)) / STACK_UNIT; // arrottondo a 2
						...
						sbStack
								.append(indent+"/* stack used only by IRQ handlers */\n"
										+ indent+"struct EE_TOS EE_x86_IRQ_tos = {\n"
										+ indent+indent+"(EE_ADDR)(&"+STACK_BASE_NAME+j+"[STACK_"+j+"_SIZE - X86_INIT_TOS_OFFSET])\n" // DELTA
										+ indent+"};\n\n");
...
//			    stackPatternFill.append("#endif\n");
				{// ORTI : Store link between task and stack
					...
					sgrCpu.setObject(SGRK_OS_STACK_VECTOR_NAME, "EE_std_system_tos");
				}
...
//			        +stackPatternFill
	...
	private void prepareMakeFile(final int currentRtosId, final IOilObjectList ool) {
	    ...
	    StringBuilder sbMakefile = new StringBuilder(commentWriterMf.writerBanner("X86"));
...
//			StringBuilder sbCommon = new StringBuilder();
		    ...
		    {	// PATHs
			    ...
			    }
	        	{
			        ...
			        String compiler_type = AbstractRtosWriter.getOsProperty(ool,X86Constants.SGRK__COMPILER_TYPE__);
			        if (X86Constants.SGRK__GCC_COMPILER__.equalsIgnoreCase(compiler_type)) {
				    	if (options.containsKey(X86Constants.PREF_X86_GCC_PATH)) {
							String tmp = (String) options.get(X86Constants.PREF_X86_GCC_PATH);
							if (tmp.length()>0) gcc = tmp;
						} 
				    	compiler_define = CommonUtils.compilerMakefileDefines(gcc, "EE_X86_GCC_DIR", wrapper);

			        } else if (X86Constants.SGRK__CLANG_COMPILER__.equalsIgnoreCase(compiler_type)) {
				    	if (options.containsKey(X86Constants.PREF_X86_CLANG_PATH)) {
							String tmp = (String) options.get(X86Constants.PREF_X86_CLANG_PATH);
							if (tmp.length()>0) gcc = tmp;
						} 
				    	compiler_define = CommonUtils.compilerMakefileDefines(gcc, "EE_X86_CLANG_DIR", wrapper);
			        } 
