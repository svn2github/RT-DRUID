 ...
 */
public class SectionWriterMcu_unibo_mparm extends SectionWriter implements IEEWriterKeywords,
	IExtractKeywordsExtentions, IGetEEOPTExtentions {

	/** Contains all properties of a UNIBO MPARM mcu (as stored inside the oil) */
	private final static String[][] UNIBO_MPARM_IRQ_KEYS = {
	//
			{"IRQ_EXT0", "/* Ext 0 */ "},
			{"IRQ_EXT1", "/* Ext 1 */"},
			{"IRQ_EXT2", "/* Ext 2 */"},
	 ...
	 * 
	 */
	public SectionWriterMcu_unibo_mparm(ErikaEnterpriseWriter parent) {
		super(Arm7Constants.DEF__unibo_mparm__,
				new String[] {
	...
	/**
	 * Write data for MCU UNIBO MPARM (only inside the first Oil Writer
	 * Buffer)
	 * 
	 ...
	 */
	protected IOilWriterBuffer[] write_unibo_mparm_MCU()
			throws OilCodeWriterException {
		...
		for (ISimpleGenRes os: ool.getList(IOilObjectList.OS)) {
			...
			if (!os.containsProperty(Arm7Constants.DEF__unibo_mparm__)) {
				...
				for (int i = 0; i < UNIBO_MPARM_IRQ_KEYS.length; i++) {
					String[] tmp = CommonUtils.getValue(vt, currentMcuPrefix
							+ UNIBO_MPARM_IRQ_KEYS[i][0]);
					if (tmp != null && tmp.length != 0) {
						irq
								.setProperty(UNIBO_MPARM_IRQ_KEYS[i][0],
										tmp[0]);
					}
				}

				// store the property inside the OS object
				os.setObject(Arm7Constants.DEF__unibo_mparm__, irq);
			}
		...
		for (int i = 0; i < UNIBO_MPARM_IRQ_KEYS.length; i++) {
			...
			// current interrupt)
			String tmp;
			if (irq.containsKey(UNIBO_MPARM_IRQ_KEYS[i][0])) {
				tmp = irq.getProperty(UNIBO_MPARM_IRQ_KEYS[i][0]);
			...
			descr = ",\t" + UNIBO_MPARM_IRQ_KEYS[i][1] + "\n";
		}

		// complete the irq table
		buffer
				.append("\t"
						+ UNIBO_MPARM_IRQ_KEYS[UNIBO_MPARM_IRQ_KEYS.length-1][1]
						+ "\n" + indent1 + "};\n");
	...
	public void updateKeywords(ArrayList<String> keywords, String[] rtosPrefix)
		...
		for (int i=0; i<rtosPrefix.length; i++) {
			...
			if (mcu_type != null) {
				...
				if ("unibo_mparm".equals(mcu_type)
						&& !keywords.contains(Arm7Constants.DEF__unibo_mparm__)) {
					keywords.add(Arm7Constants.DEF__unibo_mparm__);
				}
	...
	public List<String> getEEOpt(int type, int rtosId) {
		...
		if ((type & EE_OPT_COMMON_AUTO_ONLY) != 0) {
			...
			{ // Extract options from current keywords
				if (parent.checkKeyword(Arm7Constants.DEF__unibo_mparm__)) {
					
					answer.add("__unibo_mparm__");
					if (parent.checkKeyword(Arm7Constants.DEF__EVALUATOR7T__)) {
