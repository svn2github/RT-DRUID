...
public class PeriodSensitivityTest extends TestCase {
	...
	/***
	 * Priority assignement. In following array the task at array index 0 has highest priority, at array index 1 has second highest priority 
	 */
			...
			for(i = 0 ; i < testCases ; i++){
				for(j= 0 ; j < numTasks ; j++){
					...
					if (sensitivity > 0 ){
						...
						/********* We are adding two time precision its needed when we get EDF bound as  schedulable point ****/ 
						
						myP.setTestPeriod(j, 1 / (1/ sensitivity + 1/precision ) );
						myP.assignExistingPriorities() ;
						assertEquals(false,myP.isFPSchedulable(j));
						}
					else{
						assertEquals(false,myP.isFPSchedulable(j));
					}					
			...
			for(i = 0 ; i < testCases ; i++){
				...
				for(j= 0 ; j < numTasks ; j++){
					...
					if (sensitivity > 0)	{
						...
						myP.setTestPeriod(j, 1 / (1/ sensitivity + 1/precision) );
						myP.assignExistingPriorities();
						assertEquals(false,myP.isFPSchedulable());
						}
					else{
						assertEquals(false,myP.isFPSchedulable(j));
				}					
